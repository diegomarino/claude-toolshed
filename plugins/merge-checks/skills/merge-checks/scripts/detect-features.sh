...
detect_seeds() {
  ...
  if [[ -n "$seed_dir" ]]; then
    echo "SEEDS=true"; echo "SEEDS_DIR=$seed_dir/"; return
  fi
  ...
  if [[ -n "$seed_file" ]]; then
    echo "SEEDS=true"; echo "SEEDS_DIR=$(dirname "$seed_file")/"; return
  fi
  echo "SEEDS=false"; echo "SEEDS_DIR="
}

# ── Tests ─────────────────────────────────────────────────────────────────────
detect_tests() {
  if ls vitest.config.* &>/dev/null 2>&1; then
    echo "TESTS=true"; echo "TEST_TOOL=vitest"; return
  fi
  if ls jest.config.* &>/dev/null 2>&1 || [[ -f "babel.config.js" ]] && grep -q '"jest"' package.json 2>/dev/null; then
    echo "TESTS=true"; echo "TEST_TOOL=jest"; return
  fi
  if [[ -f "pytest.ini" ]] || [[ -f "conftest.py" ]] || [[ -f "pyproject.toml" ]] && grep -q '\[tool.pytest' pyproject.toml 2>/dev/null; then
    echo "TESTS=true"; echo "TEST_TOOL=pytest"; return
  fi
  if ls .mocharc.* &>/dev/null 2>&1; then
    echo "TESTS=true"; echo "TEST_TOOL=mocha"; return
  fi
  if [[ -f ".rspec" ]] || ls spec/spec_helper.* &>/dev/null 2>&1; then
    echo "TESTS=true"; echo "TEST_TOOL=rspec"; return
  fi
  if [[ -f "go.mod" ]]; then
    echo "TESTS=true"; echo "TEST_TOOL=go-test"; return
  fi
  echo "TESTS=false"; echo "TEST_TOOL="
}
...
detect_i18n() {
  ...
  if [[ -n "$i18n_dir" ]]; then
    echo "I18N=true"; echo "I18N_DIR=$i18n_dir"
    echo "I18N_TOOL=${tool:-other}"; return
  fi
  if [[ -n "$tool" ]]; then
    echo "I18N=true"; echo "I18N_TOOL=$tool"; echo "I18N_DIR="; return
  fi
  echo "I18N=false"; echo "I18N_TOOL="; echo "I18N_DIR="
}

# ── Typed ─────────────────────────────────────────────────────────────────────
detect_typed() {
  if ls tsconfig*.json &>/dev/null 2>&1; then
    echo "TYPED=true"; return
  fi
  if [[ -f "pyproject.toml" ]] && grep -q 'mypy\|pyright\|pytype' pyproject.toml 2>/dev/null; then
    echo "TYPED=true"; return
  fi
  if [[ -f "go.mod" ]]; then
    echo "TYPED=true"; return  # Go is always statically typed
  fi
  echo "TYPED=false"
}
...
detect_migrations() {
  ...
  if find . \( -name "drizzle.config.ts" -o -name "drizzle.config.js" \) -not -path "*/node_modules/*" -maxdepth 6 | grep -q .; then
    echo "MIGRATIONS=true"; echo "MIGRATION_TOOL=drizzle"; return
  fi
  if find . -path "*/prisma/migrations" -type d -not -path "*/node_modules/*" -maxdepth 6 | grep -q .; then
    echo "MIGRATIONS=true"; echo "MIGRATION_TOOL=prisma"; return
  fi
  if [[ -d "alembic" ]] || [[ -f "alembic.ini" ]]; then
    echo "MIGRATIONS=true"; echo "MIGRATION_TOOL=alembic"; return
  fi
  if find . -name "*.sql" -path "*/migrations/*" -maxdepth 6 -not -path "*/node_modules/*" | grep -q .; then
    echo "MIGRATIONS=true"; echo "MIGRATION_TOOL=sql-files"; return
  fi
  if ls db/migrate/ migrations/ &>/dev/null 2>&1; then
    echo "MIGRATIONS=true"; echo "MIGRATION_TOOL=other"; return
  fi
  echo "MIGRATIONS=false"; echo "MIGRATION_TOOL="
}
...
detect_routes_manual() {
  ...
  if ls next.config.* &>/dev/null 2>&1; then
    if [[ -d app || -d pages || -d src/app || -d src/pages ]]; then
      echo "ROUTES_MANUAL=false"; return
    fi
  fi
  ...
  if (( count > 0 )); then
    echo "ROUTES_MANUAL=true"; return
  fi
